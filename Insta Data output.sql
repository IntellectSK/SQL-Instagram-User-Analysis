...
describe users; -- for getting to know about the data description.
...
select t.tag_name, 
count(p.photo_id) as hashtag from photo_tags p inner join tags t on t.id = p.tag_id 
group by t.tag_name 
...
-- group by 1 means group by DATE_FORMAT((created_at) and Order by '%W'.
